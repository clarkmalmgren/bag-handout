...
4. Set **Groups** (1 group per den or per driver team; 2 to 12) and click **Solve**. Optionally adjust **Balance ±** (how far a group's house count may sit from the mean, in percent) and **Compactness** (how much solid, non-interleaved group shapes are worth). The side panel lists each group's house count, loop length, estimated minutes, street changes around the loop (a simple route has few) and difference from the mean, plus how many street names ended up split across groups.
5. Adjust by hand: click a colored street segment, pick a group to move all of that segment's houses there, or lock it. To move a single house, click its dot and pick a group number in the popup (only after a solve, since an unassigned house has no group yet); **Lock house** keeps that one house where it is (shown with a thick dark ring). To fix many houses at once (for example after a route is drawn), hold **Ctrl** and drag a rectangle on the map (Alt also works, but some Linux window managers grab it; Shift+drag is still zoom-to-box), or press **Select area** and drag without a key (Esc leaves that mode). The houses inside are highlighted with a cyan halo and a panel shows their current groups; pick a group number to move them all (one undo step), or Lock/Unlock them. Click empty map or press Esc to clear the selection; Shift+Ctrl+drag adds to it. Then **Re-optimize** to let the solver improve everything that is not locked (locked streets and locked houses). **Undo** / **Redo** (buttons, or Ctrl+Z / Ctrl+Shift+Z) step through edits.
6. Export: **Print view** opens the packet (an overview page plus one page per group with a full-page satellite map showing the route and numbered stops over the houses; addresses are not printed, they are in the CSV) with a **Print / Save as PDF** button; **Export CSV** writes group, stop order, address and coordinates; **Save project** downloads the project JSON, which contains the boundary, street data, houses, assignments, locks and config, so next year (or an offline session) starts from it.
...
Streets come from OpenStreetMap; houses come from Kane County parcel data when available (OpenStreetMap buildings are the fallback). Each house is snapped to the nearest street segment (with a preference for its own street, including across spelling differences such as "Bealer Cir"/"Bealer Circle" and small typos such as "Haladay"/"Halliday"), and walking distances between houses follow the street graph (`crossingPenalty` is the walking-distance penalty in metres for visiting the opposite side of the same street edge; creeks and ponds are handled by the walking-graph distances themselves, since houses cut off from the network are blocked, see below, plus the contiguity guard). A starting split is made by clustering house locations into balanced groups. Each group's loop is a nearest-neighbour tour improved with 2-opt and Or-opt. Simulated annealing (in a Web Worker) then moves single houses, runs of houses along a street, whole street segments and pairs of houses between groups. Group sizes must stay within the tolerance (**Balance ±**, by default 10% of the mean and never less than 2 houses, so 78 houses per group allows 71 to 85); among balanced splits it minimizes the longest group loop, plus 0.3 times the total distance, plus a **compactness** term and a small tie-break that nudges the sizes back towards the mean. The compactness term charges `weights.compact` x 30 m of walking for every one of a house's 6 nearest neighbours (straight line) that ends up in another group, which is what keeps groups solid instead of interleaved along and across streets; set **Compactness** to 0 to turn it off, raise it to prefer solid shapes over equal walking. A contiguity guard rejects any move that would increase a group's number of connected components; "connected" means houses within an 80 m walking link (`DEFAULT_LINK_DISTANCE` in `src/solver/solve.ts`), and the starting clustering uses straight-line distance, not walking distance. The random seed is fixed in the config, so the same input gives the same result. If the tolerance cannot be met the status says so and you can fix it by hand.

The group count, the balance tolerance (`weights.toleranceFrac`, shown as **Balance ±** in percent) and the compactness weight (`weights.compact`) are in the toolbar. The rest of the tuning (`crossingPenalty`, `weights.maxRoute`, `weights.total`, `weights.tolerance` as the minimum tolerance in houses, `iterations`, `seed`, `secPerHouse`, `walkSpeed`) has no UI; edit the `config` object in the saved project JSON and load it again. Projects saved by an older version load unchanged: their `tolerance` becomes the floor in houses and they pick up the current percentage and compactness defaults.
...
- Buildings with no address are still included but flagged: they are drawn with an orange outline and the panel shows a warning count. Decide for each whether it is a house. Unaddressed stops appear in the CSV as "<street> (no address)".
...
- If `addr:street` is spelled very differently from the street's `name` in OSM, the house loses its same-street snap preference and may attach to a neighbouring street. Suffixes, directionals and typos within two letters (on names of six characters or more that start with the same letter) are matched; anything looser is not, on purpose.
...
- Most tunable solver settings are JSON-only (group count, balance tolerance and compactness are in the toolbar); the group count is limited to 2 to 12.
