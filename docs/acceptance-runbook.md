...
6. **Solve for each plausible group count.** Set Groups to 6, 8 and 10 and click **Solve**. For each check: group sizes within 2 of the mean, each group's dashed loop covers one connected area, and the longest loop is not much longer than the others.
7. **Tune.** If loops are too long or straddle streets, edit `crossingPenalty`, `weights` (`maxRoute`, `total`) and `secPerHouse` in the saved project JSON's `config`, load it, and re-solve. Change one value at a time and note the effect.
